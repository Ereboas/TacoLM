...
class MegaLanguageModel(FairseqLanguageModel):
    ...
    def __init__(self, args, decoder, all_embed_tokens=None):
        super().__init__(decoder)
        self.is_ar = bool(args.ar_task)
        self.args = args
        self.supports_align_args = True
        self.all_embed_tokens = all_embed_tokens

    @staticmethod
    def add_args(parser):
        """Add model-specific arguments to the parser."""
        # fmt: off
        parser.add_argument('--nar-layer1-embedding-from-ar-checkpoint-path',
            type=str,
            default=None,)
        parser.add_argument('--text-separate-embed-positions',
            type=bool,
            default=True,)
        parser.add_argument('--bidirectional', action='store_true',
            default=False,)
        parser.add_argument('--text-full-attention', action='store_true',
            default=False,)
        parser.add_argument('--self-cross-attention', action='store_true',
            default=True,)
        ...
        parser.add_argument('--activation_dropout', type=float, metavar='D', default = 0.0)
    ...
    def build_model(cls, args, task):
        ...
        #seg_embeddings = Embedding(num_embeddings=3, embedding_dim=args.decoder_input_dim, padding_idx=0)
        ...
        if args.adaptive_input:
            ...
            if args.ar_task:
                ...
                embed_tokens = []
                if args.nar_layer1_embedding_from_ar_checkpoint_path is not None:
                    import torch
                    ar_checkpoint = torch.load("nar_layer1_embedding_from_ar_checkpoint_path")
                    ar_trained_embedding = ar_checkpoint['model']['decoder.embed_tokens.weight'].clone().detach()
                    del ar_checkpoint
                    embed_tokens.append(torch.nn.Embedding.from_pretrained(ar_trained_embedding, padding_idx=task.source_dictionary.pad()))
                else:
                    embed_tokens.append(
                        cls.build_embedding(
                            args, task.source_dictionary, args.decoder_input_dim
                ...
                for _ in range(args.rvq_layers - 2):
                    embed_tokens.append(
                        cls.build_embedding(
                            args, task.source_dictionary, args.decoder_input_dim
                        )
                ...
                import torch
                embed_tokens = torch.nn.ModuleList(embed_tokens)
        ...
        if args.ar_task:
            decoder = MegaDecoderNoCrossAttn(
                args, src_dict, embed_tokens=embed_tokens, 
                text_separate_embed_positions = args.text_separate_embed_positions,
                #seg_embeddings = seg_embeddings,
            )
        else:
            decoders = [
                MegaDecoderNoCrossAttn(
                    args, src_dict, embed_tokens=None, 
                    text_separate_embed_positions = args.text_separate_embed_positions,
                    share_cross_input_output_embed=True,
                    all_embed_tokens=embed_tokens,
                    trained_decoder_idx = i,
                    #seg_embeddings = seg_embeddings,
                )
                for i in range(1, args.rvq_layers - 1)
            ]
            decoders.append(
                MegaDecoderNoCrossAttn(
                    args, src_dict, embed_tokens=None, 
                    text_separate_embed_positions = args.text_separate_embed_positions,
                    all_embed_tokens=embed_tokens,
                    trained_decoder_idx = args.rvq_layers-1,
                    #seg_embeddings = seg_embeddings,
                )
            )
            decoders = torch.nn.ModuleList(decoders)
            return cls(args, decoders, embed_tokens)
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        kwargs['full_context_alignment'] = not self.is_ar
        kwargs['all_embed_tokens'] = self.all_embed_tokens
        kwargs['text_full_attention'] = self.args.text_full_attention
        if self.is_ar:
            return self.decoder(
                src_tokens, 
            ...
            train_layer = kwargs["train_nar_layer"]
            return self.decoder[train_layer - 1](
                src_tokens, 
                features_only=features_only,
                return_all_hiddens=return_all_hiddens,
                **kwargs
    ...
    def get_normalized_probs_scriptable(
        ...
        if self.is_ar:
            ...
            return self.decoder[0].get_normalized_probs(net_output, log_probs, sample)
    ...
    def max_positions(self):
        ...
        if self.is_ar:
            ...
            return self.decoder[0].max_positions()
...
class MegaDecoderNoCrossAttn(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens=None,
        text_separate_embed_positions=False,
        share_cross_input_output_embed=False,
        all_embed_tokens = None,
        trained_decoder_idx = None,
        #seg_embeddings = None,
    ):
        self.text_separate_embed_positions = text_separate_embed_positions
        self.share_cross_input_output_embed = share_cross_input_output_embed
        self.trained_decoder_idx = trained_decoder_idx
        ...
        input_embed_dim = embed_tokens.embedding_dim if embed_tokens is not None else all_embed_tokens[0].embedding_dim
        embed_dim = args.decoder_embed_dim
        self.embed_dim = embed_dim

        self.padding_idx = embed_tokens.padding_idx if embed_tokens is not None else all_embed_tokens[0].padding_idx
        self.max_target_positions = args.max_target_positions
        self.chunk_size = args.decoder_chunk_size
        self.attention_activation = args.attention_activation_fn

        self.embed_tokens = embed_tokens if embed_tokens is not None else None
        self.embed_scale = 1.0 if args.no_scale_embedding else math.sqrt(embed_dim) / 2
        ...
        if args.adaptive_softmax_cutoff is not None:
            ...
            assert self.trained_decoder_idx is None
        ...
        elif self.share_cross_input_output_embed:
            self.output_projection = nn.Linear(
                all_embed_tokens[self.trained_decoder_idx].weight.shape[1],
                all_embed_tokens[self.trained_decoder_idx].weight.shape[0],
                bias=False,
            )
            self.output_projection.weight = all_embed_tokens[self.trained_decoder_idx].weight
        else:
    ...
    def build_decoder_layer(self, args):
        return MegaDecoderLayer(args, no_cross_attention=True, self_cross_attention=args.self_cross_attention)
    ...
    def forward(
        ...
        text_full_attention: bool = False,
        **kwargs
    ):
        """
        Args:
        ...
        x, extra = self.extract_features(
            ...
            all_embed_tokens = all_embed_tokens,
            full_context_alignment=full_context_alignment,
            text_full_attention=text_full_attention,
        )
    ...
    def extract_features_scriptable(
        ...
        if not isinstance(prev_output_tokens, list):
            prev_output_tokens = [prev_output_tokens]

        bsz, seq_len = prev_output_tokens[0].size()
        for i in range(len(prev_output_tokens)):
            if 0 < self.chunk_size < seq_len and seq_len % self.chunk_size != 0:
                ...
                prev_output_tokens[i] = F.pad(prev_output_tokens[i], (0, num_paddings), value=self.padding_idx)
            else:
                num_paddings = 0

        # embed seg
        #seg = torch.ones_like(prev_output_tokens[0], dtype=torch.long)
        #seg[:, bpe_length:][prev_output_tokens[0][:, bpe_length:]!=1] = 2
        #seg[prev_output_tokens[0]==1] = 0

        # embed tokens
        prev_output_tokens = [
            i.contiguous()
            for i in prev_output_tokens
        ]
        if self.trained_decoder_idx is not None:
            x = 0.0
            for i in range(self.trained_decoder_idx):
                x += all_embed_tokens[i](prev_output_tokens[i])
            #x += self.seg_embeddings(seg)
            x *= self.embed_scale
        else:
            #x = self.embed_scale * (self.embed_tokens(prev_output_tokens[0]) + self.seg_embeddings(seg))
            x = self.embed_scale * self.embed_tokens(prev_output_tokens[0])
        ...
        decoder_padding_mask = prev_output_tokens[0].eq(self.padding_idx)
        ...
        for idx, layer in enumerate(self.layers):
            if x.size(0) > 1 and not full_context_alignment:
                attn_mask = self.buffered_future_mask(x)
                if text_full_attention:
                    #@params K x C x C
                    attn_mask_lower_symbol = attn_mask[-1, 0].clone()
                    attn_mask = attn_mask[None].expand(math.ceil(seq_len / self.chunk_size), -1, -1).clone()
                    attn_mask[0, :bpe_length, :bpe_length] = attn_mask_lower_symbol
            else:
                attn_mask = None

            x, layer_attn, _ = layer(x, incremental_state=incremental_state,
                                     attn_mask=attn_mask, decoder_padding_mask=decoder_padding_mask,
                                     need_attn=False, bpe_length=bpe_length)
...
@register_model_architecture("mega_lm", "mega_lm_base")
...
@register_model_architecture('mega_lm', 'mega_valle_01')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 1024)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 4096)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 4096)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 256)
    base_lm_architecture(args)

@register_model_architecture('mega_lm', 'mega_valle_02')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 1024)
    ...
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 128)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 16)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1728)
    base_lm_architecture(args)

#@params Test little-enough models.
@register_model_architecture('mega_lm', 'mega_valle_03')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    ...
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 128)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 16)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 256)
    base_lm_architecture(args)

#@params Test little-enough models.
@register_model_architecture('mega_lm', 'mega_valle_04')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 768)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 96)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 12)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 96)
    base_lm_architecture(args)

@register_model_architecture('mega_lm', 'mega_valle_05')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
    ...
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 128)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 16)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1536)
    base_lm_architecture(args)

@register_model_architecture('mega_lm', 'mega_valle_06')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 1024)
...
#@params Test little-enough models.
@register_model_architecture('mega_lm', 'mega_valle_07')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params model_size(file): 113M(noshare)
@register_model_architecture('mega_lm', 'mega_valle_08')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params model_size(file): 170M
@register_model_architecture('mega_lm', 'mega_valle_09')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    ...
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 128)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 16)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1664)
    base_lm_architecture(args)

#@params Test little-enough models.
@register_model_architecture('mega_lm', 'mega_valle_10')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params Test little-enough models.
@register_model_architecture('mega_lm', 'mega_valle_11')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params model_size(file): 104M
@register_model_architecture('mega_lm', 'mega_valle_12')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
    ...
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 128)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 16)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
    base_lm_architecture(args)

#@params model_size(file): 81M
@register_model_architecture('mega_lm', 'mega_valle_13')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
...
#@params model_size(file): 75M; 135M(pa)
@register_model_architecture('mega_lm', 'mega_valle_14')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 384)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 384)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 768)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 160)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 20)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
    base_lm_architecture(args)

#@params model_size(file): 76M
@register_model_architecture('mega_lm', 'mega_valle_15')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 384)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 384)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 768)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 196)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 24)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
    base_lm_architecture(args)

#@params model_size(file): 58M
@register_model_architecture('mega_lm', 'mega_valle_16')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 320)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 320)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 768)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 196)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 24)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
...
#@params model_size(file): 72M; fast.
@register_model_architecture('mega_lm', 'mega_pa_valle_01')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 384)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 384)
...
#@params model_size(file): 123M; 99M(newarch; noshare)
@register_model_architecture('mega_lm', 'mega_pa_valle_02')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params model_size(file): 159M(noshare)
@register_model_architecture('mega_lm', 'mega_pa_valle_03')
def mega_lm_big(args):
...
#@params model_size(file): 264M(noshare)
@register_model_architecture('mega_lm', 'mega_pa_valle_04')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 768)
    ...
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 256)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 32)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
    base_lm_architecture(args)

#@params model_size(file): 93M(newarch; noshare)
@register_model_architecture('mega_lm', 'mega_pa_valle_05')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params model_size(file): 89M(newarch; noshare)
@register_model_architecture('mega_lm', 'mega_pa_valle_06')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 512)
...
#@params model_size(file): 73M(newarch; noshare)
@register_model_architecture('mega_lm', 'mega_pa_valle_07')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 448)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 448)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 768)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 192)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 24)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
...
#@params model_size(file): 59M
@register_model_architecture('mega_lm', 'mega_pa_valle_08')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 384)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 384)
...
#@params model_size(file): 61M
@register_model_architecture('mega_lm', 'mega_pa_valle_09')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 384)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 384)
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", 768)
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 240)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 24)
    args.decoder_chunk_size = getattr(args, 'decoder_chunk_size', 1408)
    base_lm_architecture(args)

#@params model_size(file): 
@register_model_architecture('mega_lm', 'mega_pa_valle_10')
def mega_lm_big(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 320)
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", 384)
