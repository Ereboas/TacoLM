...
class TransformerLanguageModelConfig(FairseqDataclass):
    nar_layer1_embedding_from_ar_checkpoint_path: Optional[str] = field(
        default=None,
        metadata={
            "help": "Load from the AR checkpoint, to initialize the token-embedding of NAR Layer 1."
        },
    )
    text_separate_embed_positions: bool = field(
        default=True, metadata={"help": "Compute sinuous position embedding separately for (text) prompt and input tokens?"}
    )
    ...
    ar_task: bool = II("task.ar_task")
    rvq_layers: int = II("task.rvq_layers")
...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def forward(self, src_tokens, **kwargs):
        ...
        if self.is_ar:
            ...
            return self.decoder[train_layer - 1](src_tokens, **kwargs)

    def __init__(self, decoder, is_ar, all_embed_tokens=None):
        super().__init__(decoder)
        self.is_ar = bool(is_ar)
    ...
    def extract_features(self, src_tokens, **kwargs):
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        kwargs['full_context_alignment'] = not self.is_ar            # ADJUST:
        if self.is_ar:
            return self.decoder.extract_features(src_tokens, **kwargs)
        else:
            train_layer = kwargs["train_nar_layer"]
            return self.decoder[train_layer - 1].extract_features(src_tokens, **kwargs)
    ...
    def build_model(cls, args, task):
        ...
        if args.ar_task:
            ...
            decoders = [
                TransformerDecoder(
                    args, task.target_dictionary, embed_tokens=None, 
                    no_encoder_attn=True,
                    text_separate_embed_positions = args.text_separate_embed_positions,
            ...
            return cls(decoders, args.ar_task, embed_tokens)

        return cls(decoder, args.ar_task)
