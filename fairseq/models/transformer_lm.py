...
class TransformerLanguageModelConfig(FairseqDataclass):
    ...
    text_full_attention: bool = field(
        default=False, metadata={"help": "Text part uses full attention?"}
    )
...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def forward(self, src_tokens, **kwargs):
        ...
        if self.is_ar:
            return self.decoder(src_tokens,
                                full_context_alignment=not self.is_ar,
                                all_embed_tokens = self.all_embed_tokens,
                                **kwargs)
        else:
            train_layer = kwargs["train_nar_layer"]
            return self.decoder[train_layer - 1](src_tokens,
                                full_context_alignment=not self.is_ar,
                                all_embed_tokens = self.all_embed_tokens,
                                **kwargs)
    ...
    def build_model(cls, args, task):
        ...
        if args.ar_task:
            decoder = TransformerDecoder(
                ...
                text_full_attention = args.text_full_attention
            )
        else:
            decoders = [
                TransformerDecoder(
    ...
    def get_normalized_probs_scriptable(
        ...
        decoder = self.decoder if self.is_ar else self.decoder[0]
        return decoder.get_normalized_probs(net_output, log_probs, sample)
