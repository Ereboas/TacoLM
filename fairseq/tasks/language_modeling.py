...
class ModifiedLMDataset(FairseqDataset):
    ...
    def __init__(
        ...
        with open(manifest_path, "r") as f:
            self.phn_root_dir = f.readline().strip()
            self.qnt_root_dir = f.readline().strip()
        ...
        self.n_acoustic_symbols = n_acoustic_symbols
        self.n_control_symbols = n_control_symbols
    ...
    def __getitem__(self, index):
        if self.preload:
            ...
            fname_prefix = fname_prefix.as_py()
            fname_prefix = self.text_compressor.decompress(fname_prefix)
            phn_path = Path(self.phn_root_dir, fname_prefix +'.phn')
            speaker, chapter = phn_path.stem.split('-')[:2]
            qnt_path = next(Path(self.qnt_root_dir).glob(f"*/{speaker}")) / chapter / phn_path.with_suffix(".qnt").name
            phn_code_dict = torch.load(phn_path)
            qnt_code = torch.load(qnt_path)

        s, e, p = phn_code_dict['s'], phn_code_dict['e'], phn_code_dict['p']
        p = p + 4 + self.n_acoustic_symbols
        qnt_code = qnt_code + 4
        p_column_code = p.repeat(self.RVQLayer, 1)
        data_source = torch.cat([
            p_column_code,
            torch.LongTensor([self.bos]).repeat(self.RVQLayer, 1),
            qnt_code,
            torch.LongTensor([self.eos]).repeat(self.RVQLayer, 1),
        ], dim=1)

        loss_mask = torch.ones_like(data_source[0, :-1], dtype=torch.bool)
        loss_mask[:len(p)] = False

        separator = len(p) + 1 # * From separator, the qnt codes begin. 
        return {"id": index, "separator": separator, "data_source": data_source, "loss_mask": loss_mask}
    ...
    def collater(self, samples):
        ...
        if self.is_ar:
            ...
            src_tokens = data_utils.collate_tokens(
                [s["data_source"][0, :-1] for s in samples],
                self.pad,
                self.eos,
            )
            ...
            ntokens = sum(len(s["data_source"][0, :-1]) for s in samples)
            src_lengths = torch.LongTensor([s["data_source"][0, :-1].numel() for s in samples])
        else:
            train_nar_layer = torch.randint(1, self.RVQLayer, (1,)).item() #* [1, RVQLayer-1)
            src_tokens = [
                data_utils.collate_tokens(
                    [s["data_source"][i] for s in samples],
                    self.pad,
                    self.eos,
                )
            ...
            target_tokens = data_utils.collate_tokens(
                [s["data_source"][train_nar_layer] for s in samples],
                self.pad,
                self.eos,
            )
            ntokens = sum(len(s["data_source"][train_nar_layer]) for s in samples)
            src_lengths = torch.LongTensor([s["data_source"][train_nar_layer].numel() for s in samples])

        separators = torch.LongTensor([s["separator"] for s in samples])
        loss_mask = torch.zeros_like(target_tokens, dtype=torch.bool)
        for i,sample in enumerate(samples):
            loss_mask_i = sample['loss_mask']
            loss_mask[i, :loss_mask_i.size(0)] = loss_mask_i
...
class LanguageModelingConfig(FairseqDataclass):
    ...
    n_control_symbols: int = field(
        default=2000,
        metadata={"help": "size of the phoneme/bpe dictionary"},
    )
    n_acoustic_symbols: int = field(
        default=1024,
        metadata={"help": "size of the codebook"},
    )
    ar_task: bool = field(
...
class LanguageModelingTask(LegacyFairseqTask):
    ...
    def setup_dictionary(cls, args, **kwargs):
        ...
        if args.modified:
            ...
            for i in range(args.n_acoustic_symbols + args.n_control_symbols):
                dictionary.add_symbol(i)
            output_dictionary = dictionary
            logger.info(f"dictionary: codebook {args.n_acoustic_symbols} + control {args.n_control_symbols} types")
    ...
    def load_dataset(
        ...
        if self.args.modified:
            ...
            self.datasets[split] = ModifiedLMDataset(
                ...
                n_acoustic_symbols = self.args.n_acoustic_symbols,
                n_control_symbols = self.args.n_control_symbols,
            )
            return
