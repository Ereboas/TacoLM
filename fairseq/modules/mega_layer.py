...
class MegaDecoderLayer(nn.Module):
    ...
    def __init__(self, args, no_cross_attention=False, self_cross_attention=False):
        super().__init__()
        self.embed_dim = args.decoder_embed_dim
        self.self_cross_attention = self_cross_attention
        self.mega_layer = self.build_mega_layer(self.embed_dim, args)
        self.cross_attn = None if no_cross_attention else self.build_cross_attn(self.embed_dim, args)
        if self_cross_attention:
            '''
            self.text_ffn = NormalizedFeedForwardNetwork(
            ...
            self.self_cross_attn = GatedCrossAttention(
                embed_dim=self.embed_dim,
    ...
    def forward(
        ...
        **kwargs
        ...
        x, attn = self.mega_layer(x=x, padding_mask=decoder_padding_mask,
                                  ...
                                  need_weights=False, attn_mask=attn_mask, bpe_length = bpe_length)
        ...
        if self.self_cross_attn is not None:
            #text_part = self.text_ffn(x[:bpe_length])
            text_part = x[:bpe_length]
            x_, attn = self.self_cross_attn(query=x[bpe_length:], key=x[:bpe_length], value=x[:bpe_length],
                                      padding_mask=decoder_padding_mask,
                                      key_padding_mask=encoder_padding_mask,
                                      incremental_state=incremental_state,
                                      static_kv=False, need_weights=need_attn)
            x = torch.cat([
                text_part, x_
            ], dim=0)
