...
class MovingAverageGatedAttention(nn.Module):
    ...
    def __init__(
        ...
        elif rel_pos_bias == 'sin': #NOTE: # ! not implemented
            self.rel_pos_bias = PositionalEmbedding(
                max_positions, zdim, padding_idx=2
            )
        else:
...
#NOTELINE: 改为xaiver
    def reset_parameters(self):
        ...
        self.mx_proj.reset_parameters()
        self.h_proj.reset_parameters()
        nn.init.normal_(self.gamma, mean=0.0, std=std)
        nn.init.constant_(self.beta, 0.0)

    def element_attention(self, q, k, padding_mask, attn_mask, before_attn_fn, bpe_length):
        slen = k.size(2)
        ...
        # C x C
        bias = self.rel_pos_bias(slen, bpe_length=bpe_length, text_separate_embed_positions=self.text_separate_embed_positions)
        if slen != q.size(2):
            assert q.size(2) == 1
            # 1 x C
            bias = bias[-1:]
    ...
    def softmax_attention(self, q, k, padding_mask, attn_mask, before_attn_fn, bpe_length):
        slen = k.size(2)
    ...
    def forward(
        ...
        if self.attention_activation == 'softmax':
            attn_weights = self.softmax_attention(q, k, padding_mask, attn_mask, before_attn_fn, bpe_length)
        else:
            attn_weights = self.element_attention(q, k, padding_mask, attn_mask, before_attn_fn, bpe_length)
