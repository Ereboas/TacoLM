...
class SimpleRelativePositionalBias(nn.Module):
    ...
    def forward(self, seq_len, separators):
...
class RotaryRelativePositionalBias(nn.Module):
    ...
    def rotary_sep(self, x, bpe_length):
        n, d = x.size()
        x1, x2 = torch.chunk(x, 2, dim=-1)
        if self.sine is None or n > self.sine.size(0):
            self.sine, self.cosine = RotaryRelativePositionalBias.get_sinusoid_embeddings(n, d)
        ...
        n_seq = torch.arange(n, dtype=torch.int64, device=x.device)
        n_seq[bpe_length:]-= bpe_length
        sin = self.sine[n_seq]
        cos = self.cosine[n_seq]
        ret = torch.cat([x1 * cos - x2 * sin, x2 * cos + x1 * sin], dim=1)
        return ret

    def forward(self, seq_len, bpe_length=None, text_separate_embed_positions=False):
        if text_separate_embed_positions:
            a = self.rotary_sep(self.alpha.expand(seq_len, self.embed_dim), bpe_length)
            b = self.rotary_sep(self.beta.expand(seq_len, self.embed_dim), bpe_length)
