...
parser.add_argument('--cuda_id', '-c', type=int, default=0, help='ID of the CUDA device to use')
parser.add_argument('--split_id', '-s', type=int, default=-1)
parser.add_argument('--max_split_id', '-m', type=int, default=1)
parser.add_argument('--sample_p', '-p', type=float, default=0.99)
parser.add_argument('--runs', '-r', type=int, default=-1) #! set to -1 to unable multirun.
...
generate_prefix = f"./generated_all/{experiment_name}" if args.runs == -1 else f"./generated_runs/{experiment_name}"

ar_path = "./checkpoints/valle"
nar_path = "./checkpoints/valle-nar"
audio_folder_names = [f'{audio_prefix}/test_easy', f'{audio_prefix}/test_hard']
...
def generate_one(phn_path, qnt_path):
    ...
    if qnt_code.size(1) > MUL_CONST * TOO_LONG_SEC:
        return "Long"
...
for run in range(args.begin_run_idx, args.begin_run_idx+runs):
    ...
    for audio_folder_name in audio_folder_names:
        ...
        for audio_path in audio_paths:
            ...
            if True or 'test-clean' in qnt_path.parts: _ap.append(audio_path)
        audio_paths = _ap
        ...
        with progress:
            for idx, audio_path in enumerate(audio_paths):
                ...
                if False and 'test-other' in qnt_path.parts: continue
